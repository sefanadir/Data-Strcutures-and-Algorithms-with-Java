...
public class BTree<E extends Comparable<E>> implements SearchTree<E> {
    ...
    private static class Node<E> {
         ...
         * < data[i] for i < size
         *  and to the subtree of children > data[size-1] for i == size
         */
        private final Node<E>[] child;

        /**
         * Create an empty node of size n*n
         *
    ...
    private int binarySearch(E item, E[] data, int startSize, int rootSize) {
        if (startSize >= rootSize) {
            return startSize;
        } else {
            int middleIndex = (startSize + rootSize) / 2;
            int compare = item.compareTo(data[middleIndex]);
            if (compare == 0) {
                return middleIndex;
            } else if (compare < 0) {
                return binarySearch(item, data, startSize, middleIndex);
            } else {
                return binarySearch(item, data, middleIndex + 1, rootSize);
            }
        }
    }

    /**
     * Recursive insert an Object into the Tree. Inserted item must implement
